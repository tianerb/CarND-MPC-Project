      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // Transform reference points from map to car coordinates
          Eigen::VectorXd car_x(ptsx.size());
          Eigen::VectorXd car_y(ptsx.size());
          ...
          double cte = polyeval(coeffs, x_late);
          ...
          for (int i = 0; i < mpc_output[2]; i++) // mpc_output[2] = N
          {
            mpc_x_vals.push_back(mpc_output[i * 2 + 3]);
            mpc_y_vals.push_back(mpc_output[i * 2 + 4]);
          }
